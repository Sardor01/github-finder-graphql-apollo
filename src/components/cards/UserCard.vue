...
import type { NonNullableArray } from '~/types'

defineProps<{
  item: NonNullableArray<NonNullableArray<NonNullableArray<SearchUsersQuery['search']['edges']>[number]>['node']>
}>()
</script>
...
<template>
  ...
  >
    ...
    <RouterLink v-slot="{ href, navigate }" :to="`/users/${item.login}`" custom>
      <BaseButton is="a" :href="href" @click="navigate"> More </BaseButton>
    </RouterLink>
  </div>
</template>
